...
| Web View | Excel File |
| -------------- | --------------- |
| ![Points and Placement in the Battle History are marked](assets/NinjalaTournamentStatsWebView.png "Battle History") | ![Excel file output when "Solo" is selected](assets/NinjalaTournamentStats.png "Sample Excel file") |

* The tool itself can be found on the GitLab page: https://gitlab.com/InitialY/tournamentstatscreator

# How to use
Visit the website https://initialy.github.io/, fill in the fields, and click the extract button. Once the process is complete, you can directly view your statistics in web or  download them as Excel file.
...
# How does it work
First, the tool filters and sorts the provided screenshots based on the timestamp in the meta data. If no timestamp are found, default timestamps are generated. If there are duplicates, they will be removed. 

After that, the images will be paired so that the correct Battle History image and Detailed Result image are matched together. This pairing is done using the timestamp and a sample extraction.

Once everything is prepared, the extraction begins. This step does not use the common method known as OCR (Optical Character Recognition) but rather a more tailored approach. This was necessary because the in-game font caused numerous issues. The extraction checks six fixed spots on the image pairs (depicted below), comparing these regions with cutout templates to assess their similarity. Based on the results, the tool identifies the correct values. These values are stored in a specific structure, which are then written into an Excel file. Excel tables and functions are utilized, making it easy to modify the file afterward in case the tool misidentified something or to add missing data.
...
The output Excel file contains either two or three charts, depending on the selected tournament format. These charts outline the Points Course, Placement Distribution Frequency, and Win Rate. Since the "Team" tournament format does not have a placement, only the Points Course and Win Rate are included.
